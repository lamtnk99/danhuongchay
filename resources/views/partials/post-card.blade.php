<article class="group overflow-hidden rounded-2xl border border-emerald-900/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <div class="p-5">
        <div class="flex flex-wrap items-center gap-3 text-xs font-medium text-stone-500">
            <span class="rounded-full bg-amber-50 px-3 py-1 text-amber-800">{{ $post->category->localized('name') }}</span>
        ...
        <h3 class="mt-4 text-xl font-semibold leading-snug text-emerald-950">
            <a href="{{ localized_route('blog.show', ['slug' => $post->localizedSlug()]) }}" class="hover:text-emerald-700">{{ $post->localized('title') }}</a>
        </h3>
        <p class="mt-3 line-clamp-3 text-sm leading-6 text-stone-600">{{ $post->localized('excerpt') }}</p>
    </div>
</article>
